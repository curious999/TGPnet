...
data_root_folder = './human/data/ATR/test_images';
output_mat_folder = fullfile('./human/features/TGPnet/test/mask/');
save_result_folder = fullfile('./human/features/TGPnet/test/results');
...
for i = 1 : numel(output_dir)
    ...
    img = imread(fullfile(data_root_folder, [img_fn, '.png']));
    img_row = size(img, 1);
    img_col = size(img, 2);
