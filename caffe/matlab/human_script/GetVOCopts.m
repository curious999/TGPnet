...
VOCopts.imgpath=[seg_root '/dataset/images/%s.jpg'];
VOCopts.imgsetpath=[seg_root '/dataset/%s.txt'];
...
%else
    VOCopts.seg.clsimgpath=[seg_root '/dataset/segmentations/%s.png'];
%end

VOCopts.seg.instimgpath=[seg_root '/SegmentationObject/%s.png'];
VOCopts.seg.imgsetpath=[seg_root '/dataset/%s.txt'];
...
if ~isempty(strfind(seg_root, 'human'))
  VOCopts.classes={...
    'hat'
    'hair'
    'glove'
